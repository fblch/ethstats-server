...
History.prototype.add = function(block, id, trusted, addingHistory)
{
	...
	// Older versions of geth will not send block.votes
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && /*!_.isUndefined(block.votes) &&*/ !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
...
History.prototype.getVotesCount = function()
{
	// Create an array containing the votes values of the latest 50 blocks
	var votes = _( this._items )
		.orderBy( 'height', "desc" )
		// .filter(function (item)
		// {
		// 	return item.block.trusted;
		// })
		.slice(0, MAX_VOTES)	// Limit to only the latest 50 blocks of information.
		.map(function (item)
		{
			return item.block.votes ? item.block.votes.length : 0;
		})
		.value();

	// console.log("votes: ", votes);

	var votesCount = votes.reduce((acc, num) => acc + num, 0);	// Sum the votes values of the latest 50 blocks
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		var chartHistory = _( this._items )
			...
			.map(function (item)
			{
				return {
					...
					votes: item.block.votes ? item.block.votes.length : 0,
		...
		this._callback(null, {
			...
			votes : _.map( chartHistory, 'votes' ),
